...
def hello_pubsub(event, context):
    ...
    try:
        if(temp["status"] in ["QUEUED", "SUCCESS", "WORKING", "TIMEOUT", "CANCELLED", "FAILED", "FAILURE"]):
            requests.get(
                "https://us-central1-mle-by-xjl.cloudfunctions.net/sendmsg?message=__"+str(datetime.now())[0:-7]+"__%0ABuild%20Job%20Status%20Updated%0AImage%20:"+str(temp["images"])+"%0AStatus:%20"+str(temp["status"]))
        if(temp["status"] == "SUCCESS"):
            ...
            try:
                ...
                requests.get("https://us-central1-mle-by-xjl.cloudfunctions.net/sendmsg?message=__" +
