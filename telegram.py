    ...
    if request.method == "POST":
        try:
            ...
            if str(update.message.text) == "/crawl":
                ...
                # Reply with the same message
                bot.sendMessage(
                    chat_id=chat_id, text="https://mle-be-zolecwvnzq-uc.a.run.app/minplot.png?uuid="+str(uuid.uuid4()))
                bot.sendMessage(
                    chat_id=chat_id, text="https://mle-be-zolecwvnzq-uc.a.run.app/maxplot.png?uuid="+str(uuid.uuid4()))
            else:
                bot.sendMessage(
